          ...
          <Grid size={{ xs: 12, md: 6 }}>
            <Card variant="outlined" sx={{ p: 3, height: '100%' }}>
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 2 }}>
                <TextField label="Nomi" {...register('name')} error={!!errors.name} helperText={errors.name?.message} fullWidth />
                <TextField label="SKU" {...register('sku')} disabled={isEdit} error={!!errors.sku} helperText={errors.sku?.message} fullWidth />
                <TextField select label="Kategoriya" {...register('categoryId')} error={!!errors.categoryId} helperText={errors.categoryId?.message} fullWidth>
                  {categories.map((c) => (
                    <MenuItem key={c.id} value={c.id}>{c.name}</MenuItem>
                  ))}
                </TextField>
                <TextField label="Shtrix-kod" {...register('barcode')} helperText="Skaner yoki qo'lda kiriting" fullWidth />
          ...
          <Grid size={{ xs: 12, md: 6 }}>
            <Card variant="outlined" sx={{ p: 3, display: 'flex', flexDirection: 'column', height: '100%' }}>
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                <TextField label="PDF Katalog URL" {...register('pdfCatalogUrl')} fullWidth />
                <TextField label="Texnik Passport URL" {...register('techPassportUrl')} fullWidth />
                <TextField label="Qo'llanma (User Manual) URL" {...register('userManualUrl')} fullWidth />
              </Box>
            </Card>
          </Grid>
